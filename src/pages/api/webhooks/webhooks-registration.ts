...
const commonHandler = function (topic, shop, webhookRequestBody) {
  console.log(
    `inside the webhook handler added using addHandler. topic ${topic}, shop: ${shop}, webhookRequestBody: ${webhookRequestBody}`
  );
};

const registerMyWebhooks = async function (shop, accessToken) {
  console.log(
    `Going to register webhooks. Shop ${shop}, accessToken: ${accessToken}`
  );
